...
// //getting skills according to career
// app.post('/api/careersFetch', (req, res) => {
//   const careerLabel = req.body;
//     console.log(careerLabel);
//   // Find the occupation (career) by label
//   const occupation = Array.from(data.occupationsMap.values())
//     .find(o => o.id == careerLabel);
    
//   if (!occupation) {
//     return res.status(404).json({ error: 'Career not found' });
//   }

//   // Get essential + optional skills IDs
//   const occupationSkills = data.occupationsToSkills[occupation.ID] || { essential: [], optional: [] };
//   const allSkillIds = [...occupationSkills.essential, ...occupationSkills.optional];

//   // Convert IDs into skill labels
//   const skills = allSkillIds
//     .map(id => data.skillsMap.get(id))
//     .filter(s => s) // remove nulls
//     .map(s => s.PREFERREDLABEL);

//   res.json({
//     career: occupation.PREFERREDLABEL,
//     skills: skills
//   });
// });

// getting skills according to career (robust version)
app.post('/api/careersFetch', (req, res) => {
  // expect body: { id: "OCCUPATION_ID" }  OR  { label: "Software developer" }
  const { career, label } = req.body || {};
  console.log('POST /api/careersFetch body:', req.body);
    const id = career
    console.log(id)
  if (!id) {
    return res.status(400).json({ error: 'Please provide either { id: "..."} or { label: "..." } in the request body' });
  }

  // Try to find by ID first (fast, uses the Map)
  let occupation = null;
  if (id) {
    // occupationsMap was built with keys = o.ID
    occupation = data.occupationsMap.get(String(id));
    // fallback: sometimes the map keys might be numbers/strings mismatch
    if (!occupation) {
      occupation = Array.from(data.occupationsMap.values()).find(o => String(o.ID) === String(id));
    }
  } else if (label) {
    const labelLower = String(label).toLowerCase();
    occupation = Array.from(data.occupationsMap.values()).find(o => (o.PREFERREDLABEL || '').toLowerCase() === labelLower);
  }
  ...
  // Convert IDs into skill objects (id + label)
  const skills = allSkillIds
...
// POST /api/careerFromSkills
// Body: { skills: ["skillID1", "skillID2", ...] }
// POST /api/careerFromSkills
// Body: { skills: ["SK001", "SK002", ...] }   <-- skill IDs only
app.post('/api/careerFromSkills', (req, res) => {
  const skills = req.body;
    console.log(skills)
  if (!Array.isArray(skills) || skills.length === 0) {
    return res.status(400).json({ error: 'Please provide an array of skill IDs in "skills".' });
  }

  // Normalize input skill IDs to strings to avoid type mismatch
  const inputSkills = skills.map(s => String(s));

  let bestMatch = null;
  let maxMatchCount = -1;            // <- important: start at -1 so even 0 matches will pick first occupation

  for (const occupation of data.occupationsMap.values()) {
    const occSkills = data.occupationsToSkills[occupation.ID] || { essential: [], optional: [] };
    // normalize occupation skill ids too
    const allOccSkillIds = new Set([
      ...occSkills.essential.map(String),
      ...occSkills.optional.map(String)
    ]);

    const matched = inputSkills.filter(id => allOccSkillIds.has(id));

    if (matched.length > maxMatchCount) {
      maxMatchCount = matched.length;
      bestMatch = { occupation, matchedSkillIds: matched };
    }
    // note: strict > keeps the first-encountered occupation when there's a tie
  }

  if (!bestMatch) {
    // This should only happen if there are no occupations loaded
    return res.status(500).json({ error: 'No occupations available to match.' });
  }

  // Build output: all skills for the chosen career + matched skill objects
  const occSkills = data.occupationsToSkills[bestMatch.occupation.ID] || { essential: [], optional: [] };
  const allSkillIds = [...occSkills.essential, ...occSkills.optional].map(String);
  ...
  const matchedSkills = bestMatch.matchedSkillIds
    .map(id => {
      const s = data.skillsMap.get(id);
      return s ? { id: s.ID, label: s.PREFERREDLABEL } : { id };
    });

  res.json({
    career: {
      id: bestMatch.occupation.ID,
      label: bestMatch.occupation.PREFERREDLABEL,
      description: bestMatch.occupation.DESCRIPTION || bestMatch.occupation.DEFINITION || ''
    },
